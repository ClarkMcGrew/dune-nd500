...
class Geant4(CMakePackage):
    ...
    version('9.6.p01', '9e6e08da2a04c122e46835193ed8a96e')
    
    variant('qt', default=True, description='Enable Qt support')
    variant('debug', default=False, description='Build debug version')
    variant('zlib', default=True, description='Use system zlib')
    variant('usolids', default=False, description='Use experimental usolids')
    variant('data', default=True, description='Automatically get data files')
    ...
    depends_on("clhep@2.2.0.4~cxx11+cxx14", when="@9.6.p01")
    depends_on("expat")
    depends_on("zlib")
    depends_on("vecgeom")
    depends_on("xerces-c")
    ...
    def cmake_args(self):
        ...
        # Choose the highest C++ standard that works.
        if 'geant4@9' in spec:
            print "using cxx11"
            options.append('-DGEANT4_BUILD_CXXSTD=c++11')            
        elif '+cxx14' or '+cxx1y' in spec:
        ...
        if '+usolids' in spec:
            ...
            options.append('-DGEANT4_USE_USOLIDS=OFF')
        ...
        if '+qt' in spec:
            ...
            options.append('-DQT_QMAKE_EXECUTABLE=%s' %
                           spec['qt'].prefix + '/bin/qmake')
        else:
            options.append('-DGEANT4_USE_QT=OFF')
